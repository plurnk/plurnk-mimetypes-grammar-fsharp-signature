#!/usr/bin/env node
// CI byte-identical reproducibility check for fsharp-signature.wasm.
...
const committed = await readFile(path.join(repoRoot, "fsharp-signature.wasm"));
const committedHash = createHash("sha256").update(committed).digest("hex");
console.log(`committed fsharp-signature.wasm sha256: ${committedHash}`);

const work = await mkdtemp(path.join(tmpdir(), "grammar-fsharp-signature-verify-"));
...
console.log(`rebuilt fsharp-signature.wasm sha256: ${rebuiltHash}`);
