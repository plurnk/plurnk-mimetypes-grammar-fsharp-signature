#!/usr/bin/env node
// Reproducible WASM build for tree-sitter-fsharp-signature.
...
const work = await mkdtemp(path.join(tmpdir(), "grammar-fsharp-signature-build-"));
await run("git", ["clone", "--no-checkout", "https://github.com/ionide/tree-sitter-fsharp.git", "src"], { cwd: work });
...
const buildCwd = path.join(work, "src", "fsharp_signature");
await run(cli, ["generate"], { cwd: buildCwd });
await run(cli, ["build", "--wasm"], { cwd: buildCwd });

// Locate produced wasm and copy as fsharp-signature.wasm.
const fs = await import("node:fs/promises");
const built = (await fs.readdir(buildCwd)).find((f) => f.endsWith(".wasm"));
if (!built) throw new Error("no .wasm produced");
await copyFile(path.join(buildCwd, built), path.join(repoRoot, "fsharp-signature.wasm"));
const bytes = (await readFile(path.join(repoRoot, "fsharp-signature.wasm"))).length;
console.log(`fsharp-signature.wasm: ${bytes} bytes (built from ${pin})`);
